...
# for i in range(0, 10):
#     print(lda.show_topic(i, topn=20))

# plot topic 0
topic_data = {"words":[], "weights":[]}
for word, weight in lda.show_topic(0, topn=20):
...
# plot topic weights for specific document
...
corpus_lda = np.sum(np.array([lda.get_document_topics(processed_corpus[i], minimum_probability=0.0)[1] for i in range(10)]), axis=1)
corpus_data = {"topic_num":[i for i in range(10)], "topic_weight":corpus_lda}
corpus_df = pd.DataFrame(corpus_data)
fig = px.bar(corpus_df, x="topic_num", y="topic_weight")
fig.show()
# top_topics = lda.top_topics(processed_corpus)
# print(top_topics)